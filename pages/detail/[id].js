...
export default function Pokemon({ data }) {
  ...
  const About = ({ data }) => {
    return (
      <div className="grid grid-cols-5 capitalize gap-y-2 font-medium text-sm">
  ...
  const Stats = ({ stats }) => {
    return (
      <div key={stats.stats} className="w-full text-sm">
        {stats.stats.map((e, i) => (
          <div className="grid grid-cols-5 capitalize mb-2" key={i}>
          ...
          <div className="flex text-sm justify-between font-medium">
            {lists.map((e) => (
              <div
                ...
                className={`transition-all duration-300 text-black text-opacity-50 ${
